 ...
 * An item is owned by this block if its version is equal to the expected
 * version, otherwise it has been processed by another thread and possibly
 * reused.
 *
 * A block is always of capacity 2^i, i \in N_0. For all owned items, if the
 * index i < j then i.key < j.key.
 */
...
class block {
 public:
  ...
  /** Information about a specific item. A nullptr item denotes failure of the
   * operation. */
 ...
 public:
  ...
  void insert(item<K, V> *it, const version_t version);
  void insert_tail(item<K, V> *it, const version_t version);
  void merge(const block<K, V> *lhs, const block<K, V> *rhs);
  void merge(const block<K, V> *lhs, const size_t lhs_first,
             const block<K, V> *rhs, const size_t rhs_first);
  void copy(const block<K, V> *that);

  /** Returns null if the block is empty, and a peek_t struct of the minimal
   * item otherwise. Removes observed unowned items from the current block. */
  peek_t peek();
  peek_t peek(size_t &ix, const size_t first);

  /** Iterates the block from last to first and sets key to the first key it
   * finds. If none are found, returns false. */
 ...
 private:
   ...
   *  Since the dist LSM is concurrent and other threads can take items without
   * the owning thread knowing about it, size is not an exact value. Instead, it
   * counts the number of elements that were written into the local list of
   * items by the owning thread, even if those items currently aren't active
   * anymore. */
  size_t m_last;
