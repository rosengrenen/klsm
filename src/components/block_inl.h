...
typename block<K, V>::peek_t block<K, V>::spying_iterator::next() {
...
template <class K, class V>
void block<K, V>::insert(item<K, V> *it, const version_t version) {
  assert(m_first == 0);
  assert(m_last == 0);
...
template <class K, class V>
void block<K, V>::insert_tail(item<K, V> *it, const version_t version) {
  assert(m_used);
...
template <class K, class V>
void block<K, V>::merge(const block<K, V> *lhs, const block<K, V> *rhs) {
  merge(lhs, lhs->m_first, rhs, rhs->m_first);
}

template <class K, class V>
void block<K, V>::merge(const block<K, V> *lhs, const size_t lhs_first,
                        const block<K, V> *rhs, const size_t rhs_first) {
  /* The following assertions are no longer valid since we now sometimes merge
  blocks
  ...
  const size_t skipped_prunes =
      std::max(lhs->m_skipped_prunes, rhs->m_skipped_prunes);
...
template <class K, class V>
void block<K, V>::copy(const block<K, V> *that) {
  assert(m_used);
  assert(m_first == 0);
  assert(m_last == 0);
...
template <class K, class V>
typename block<K, V>::peek_t block<K, V>::peek() {
  size_t ix;
  return peek(ix, m_first);
}

template <class K, class V>
typename block<K, V>::peek_t block<K, V>::peek(size_t &ix, const size_t first) {
...
template <class K, class V>
const typename block<K, V>::block_item *block<K, V>::peek_nth(
...
template <class K, class V>
bool block<K, V>::peek_tail(K &key) {
...
template <class K, class V>
typename block<K, V>::spying_iterator block<K, V>::iterator() {
...
template <class K, class V>
size_t block<K, V>::first() const {
  return m_first;
}

template <class K, class V>
size_t block<K, V>::last() const {
  return m_last;
}

template <class K, class V>
size_t block<K, V>::size() const {
  return m_last - m_first;
}

template <class K, class V>
size_t block<K, V>::power_of_2() const {
  return m_power_of_2;
}

template <class K, class V>
size_t block<K, V>::capacity() const {
  return m_capacity;
}

template <class K, class V>
bool block<K, V>::used() const {
  return m_used;
}

template <class K, class V>
void block<K, V>::set_unused() {
  assert(m_used);
  m_used = false;
...
template <class K, class V>
void block<K, V>::set_used() {
  assert(!m_used);
  m_used = true;
}

template <class K, class V>
bool block<K, V>::item_owned(const block_item &block_item) {
