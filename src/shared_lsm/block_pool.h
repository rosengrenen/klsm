...
class block_pool {
 ...
 public:
  ...
  /** Given the current version of the global array, returns a block of capacity
   * 2^i. */
  ...
  void publish(block<K, V> **blocks, const size_t nblocks,
  ...
  void free_local() { free_local_except(nullptr); }
  ...
  bool contains(const block<K, V> *block) const { return (find(block) != -1); }
 ...
 private:
  ...
  static constexpr int ix(const size_t i) { return BLOCKS_PER_LEVEL * i; }
