...
template <class K, class V, int Rlx>
void shared_lsm_local<K, V, Rlx>::insert(
    const K &key, const V &val, versioned_array_ptr<K, V, Rlx> &global_array) {
...
template <class K, class V, int Rlx>
void shared_lsm_local<K, V, Rlx>::insert(
    block<K, V> *b, versioned_array_ptr<K, V, Rlx> &global_array) {
...
template <class K, class V, int Rlx>
void shared_lsm_local<K, V, Rlx>::insert_block(
    block<K, V> *b, versioned_array_ptr<K, V, Rlx> &global_array) {
  ...
  while (true) {
    ...
    auto &new_blocks =
        (observed_version & 1) ? m_array_pool_evens : m_array_pool_odds;
    ...
    if (observed_version == global_array.version() &&
        global_array.compare_exchange_strong(observed_packed, new_blocks)) {
      m_block_pool.publish(new_blocks_ptr->m_blocks, new_blocks_ptr->m_size,
...
template <class K, class V, int Rlx>
bool shared_lsm_local<K, V, Rlx>::delete_min(
    V &val, versioned_array_ptr<K, V, Rlx> &global_array) {
...
template <class K, class V, int Rlx>
void shared_lsm_local<K, V, Rlx>::peek(
    typename block<K, V>::peek_t &best,
    versioned_array_ptr<K, V, Rlx> &global_array) {
  if (local_array_copy_is_fresh(global_array) && !m_cached_best.empty() &&
      !m_cached_best.taken()) {
...
template <class K, class V, int Rlx>
bool shared_lsm_local<K, V, Rlx>::local_array_copy_is_fresh(
    versioned_array_ptr<K, V, Rlx> &global_array) const {
  return (m_local_array_copy.version() == global_array.version());
}

template <class K, class V, int Rlx>
void shared_lsm_local<K, V, Rlx>::refresh_local_array_copy(
    block_array<K, V, Rlx> *&observed_packed, version_t &observed_version,
    versioned_array_ptr<K, V, Rlx> &global_array) {
