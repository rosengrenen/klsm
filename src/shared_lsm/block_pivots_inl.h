...
block_pivots<K, V, Rlx, MaxBlocks>::operator=(
    const block_pivots<K, V, Rlx, MaxBlocks> &that) {
...
template <class K, class V, int Rlx, int MaxBlocks>
size_t block_pivots<K, V, Rlx, MaxBlocks>::shrink(block<K, V> **blocks,
                                                  const size_t size) {
  ...
  return resize(1, best.m_key, m_maximal_pivot, blocks, size);
}

template <class K, class V, int Rlx, int MaxBlocks>
size_t block_pivots<K, V, Rlx, MaxBlocks>::grow(const int initial_range_size,
                                                block<K, V> **blocks,
                                                const size_t size) {
  COUNT_INC(pivot_grows);

  return resize(initial_range_size, m_maximal_pivot,
                std::numeric_limits<K>::max(), blocks, size);
}

#define CORRECTED_TENTATIVE_COUNT() \
  (elements_in_tentative_range + 1 - elements_with_maximal_key)

template <class K, class V, int Rlx, int MaxBlocks>
size_t block_pivots<K, V, Rlx, MaxBlocks>::resize(const int initial_range_size,
  ...
  // We could skip owned() checks by keeping a bitmap around - once an item is
  // taken, it is taken forever.
  ...
  while (true) {
    ...
    // the number of items with the maximal encountered key - all but one of
    // these may be ignored for the sake of relaxation bounds.
...
out:
  ...
  /* Note that m_count >= elements_in_tentative_range >= actual # elements in
   * range. We return m_count in this case s.t. peek() can assign a random
   * element out of the full available range. */
  return m_count;
}
...
template <class K, class V, int Rlx, int MaxBlocks>
size_t block_pivots<K, V, Rlx, MaxBlocks>::count(const size_t size) {
...
template <class K, class V, int Rlx, int MaxBlocks>
size_t block_pivots<K, V, Rlx, MaxBlocks>::count_in(
...
template <class K, class V, int Rlx, int MaxBlocks>
size_t block_pivots<K, V, Rlx, MaxBlocks>::nth_ix_in(
    const size_t relative_element_ix, const size_t block_ix) const {
  assert(block_ix < MaxBlocks);
  return m_lower[block_ix] + relative_element_ix;
}

template <class K, class V, int Rlx, int MaxBlocks>
void block_pivots<K, V, Rlx, MaxBlocks>::mark_first_taken_in(
...
template <class K, class V, int Rlx, int MaxBlocks>
int block_pivots<K, V, Rlx, MaxBlocks>::pivot_of(block<K, V> *block) const {
...
template <class K, class V, int Rlx, int MaxBlocks>
void block_pivots<K, V, Rlx, MaxBlocks>::insert(const size_t block_ix,
                                                const size_t size,
                                                const int first_in_block,
                                                const int pivot) {
  memmove(&m_upper[block_ix + 1], &m_upper[block_ix],
          sizeof(m_upper[0]) * (size - block_ix));
  memmove(&m_lower[block_ix + 1], &m_lower[block_ix],
          sizeof(m_lower[0]) * (size - block_ix));
  set(block_ix, first_in_block, pivot);
}

template <class K, class V, int Rlx, int MaxBlocks>
void block_pivots<K, V, Rlx, MaxBlocks>::set(const size_t block_ix,
                                             const int first_in_block,
                                             const int pivot) {
...
template <class K, class V, int Rlx, int MaxBlocks>
void block_pivots<K, V, Rlx, MaxBlocks>::copy(const size_t src_ix,
                                              const size_t dst_ix) {
  if (src_ix == dst_ix) {
    return;
  }
