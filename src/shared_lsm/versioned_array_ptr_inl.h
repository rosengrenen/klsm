...
template <class K, class V, int Rlx, int Algn>
bool versioned_array_ptr<K, V, Rlx, Algn>::matches(block_array<K, V, Rlx> *ptr,
                                                   version_t version) {
  return ((intptr_t)ptr & MASK) == (version & MASK);
}

template <class K, class V, int Rlx, int Algn>
block_array<K, V, Rlx> *versioned_array_ptr<K, V, Rlx, Algn>::packed_ptr(
...
template <class K, class V, int Rlx, int Algn>
block_array<K, V, Rlx> *versioned_array_ptr<K, V, Rlx, Algn>::unpacked_ptr(
    block_array<K, V, Rlx> *ptr) {
  const intptr_t intptr = (intptr_t)ptr;
...
template <class K, class V, int Rlx, int Algn>
block_array<K, V, Rlx> *versioned_array_ptr<K, V, Rlx, Algn>::load() {
  return unpacked_ptr(load_packed());
}

template <class K, class V, int Rlx, int Algn>
version_t versioned_array_ptr<K, V, Rlx, Algn>::version() {
  return load()->version();
}

template <class K, class V, int Rlx, int Algn>
block_array<K, V, Rlx> *versioned_array_ptr<K, V, Rlx, Algn>::load_packed() {
  return m_ptr.load(std::memory_order_relaxed);
}

template <class K, class V, int Rlx, int Algn>
bool versioned_array_ptr<K, V, Rlx, Algn>::compare_exchange_strong(
    block_array<K, V, Rlx> *&expected_packed,
    aligned_block_array<K, V, Rlx, Algn> &desired) {
  return m_ptr.compare_exchange_strong(
      expected_packed, packed_ptr(desired.ptr()), std::memory_order_relaxed);
}
