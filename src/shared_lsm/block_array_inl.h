...
template <class K, class V, int Rlx>
void block_array<K, V, Rlx>::block_insert(const size_t block_ix,
                                          block<K, V> *block) {
  memmove(&m_blocks[block_ix + 1], &m_blocks[block_ix],
...
template <class K, class V, int Rlx>
void block_array<K, V, Rlx>::block_set(const size_t block_ix,
                                       block<K, V> *block) {
...
template <class K, class V, int Rlx>
void block_array<K, V, Rlx>::insert(block<K, V> *new_block,
                                    block_pool<K, V> *pool) {
  if (m_size == 0) {
    ...
    /* Merge with equal capacity blocks until strictly descending invariant is
     * preserved.*/
    ...
    for (; i > 0; i--) {
      ...
      if (other_block == nullptr) {
        ...
        merged_block->merge(insert_block, insert_block->first(), other_block,
                            other_first);
  ...
  /* If the number of elements within the pivot range is smaller than our lower
   * bound, attempt to improve pivots. */

  const size_t ncandidates = m_pivots.count(m_size);
  if (ncandidates > Rlx + 1) {
    // TODO: Possibly a more efficient reset mechanism which uses knowledge of
    // existing pivots.
...
template <class K, class V, int Rlx>
void block_array<K, V, Rlx>::compact(block_pool<K, V> *pool) {
  remove_null_blocks();
...
template <class K, class V, int Rlx>
void block_array<K, V, Rlx>::remove_null_blocks() {
#ifndef NDEBUG
...
template <class K, class V, int Rlx>
bool block_array<K, V, Rlx>::delete_min(V &val) {
...
template <class K, class V, int Rlx>
typename block<K, V>::peek_t block_array<K, V, Rlx>::peek() {
...
template <class K, class V, int Rlx>
void block_array<K, V, Rlx>::copy_from(const block_array<K, V, Rlx> *that) {
  do {
