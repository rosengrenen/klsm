...
itree::itree() : m_root(nullptr), m_size(0) {}

itree::~itree() { itree_free(m_root); }

int itree::insert(const elem_t &index) {
  uint64_t holes = 0;
...
int itree::erase(const elem_t &index, uint64_t *rank) {
...
void itree::print() { _itree_print(m_root, 0); }

void itree::_itree_print(const itree_t *root, uint8_t level) {
  if (root == NULL) {
    return;
  }

  printf("%*s{ k: %llu, v: %llu, h: %d }\n", level * INDENT, "",
         (long long unsigned)root->k.key, (long long unsigned)root->v, root->h);

  _itree_print(root->l, level + 1);
  _itree_print(root->r, level + 1);
}

int itree::_itree_new_node(const elem_t &index, itree_t **root) {
...
inline int8_t itree::_itree_height(const itree_t *node) {
  return (node == NULL) ? -1 : node->h;
}

inline void itree::_itree_set_height(itree_t *node) {
 ...
 */
inline uint64_t itree::_itree_count(const itree_t *root) {
  if (root == NULL) {
    return 0;
  }
 ...
 */
void itree::_itree_rebalance(itree_t **root) {
  itree_t *droot = *root;
...
int itree::_itree_descend_l(const elem_t &index, itree_t **root,
                            uint64_t *holes) {
  itree_t *droot = *root;
...
int itree::_itree_descend_r(const elem_t &index, itree_t **root,
                            uint64_t *holes) {
  itree_t *droot = *root;
 ...
 */
int itree::_itree_insert(const elem_t &index, itree_t **root, uint64_t *holes) {
  itree_t *droot = *root;
  int ret = 0;
...
int itree::_itree_erase(const elem_t &index, itree_t **root, uint64_t *rank) {
  itree_t *droot = *root;
  int ret = 0;
...
void itree::itree_free(itree_t *root) {
  if (root == NULL) {
    return;
  }
...
#define ITER_PUSH(iter, val)        \
  do {                              \
    iter->stack[iter->top++] = val; \
  } while (0);
#define ITER_POP(iter) (iter->stack[--iter->top])

/* itree_iter definitions. */

itree::itree_iter_t *itree::itree_iter_init(itree_t *root) {
...
itree::itree_t *itree::itree_iter_next(itree_iter_t *iter) {
  if (iter->top == 0) {
    return NULL;
  }
...
void itree::itree_iter_free(itree_iter_t *iter) { free(iter); }
