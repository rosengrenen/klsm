 ...
 * Split: 50% of threads perform inserts, 50% of threads perform deletes (in
 * case of an odd thread count there are more inserts than deletes). Producer: A
 * single thread performs inserts, all others delete.
 */
enum {
  WORKLOAD_UNIFORM = 0,
  WORKLOAD_SPLIT,
 ...
 * Ascending: Keys are generated uniformly at random within a smaller integer
 * range [x, x + z] s.t. x rises over time.
 */
enum {
  KEYS_UNIFORM = 0,
  KEYS_ASCENDING,
...
struct settings {
  ...
  bool are_valid() const {
    if (nthreads < 1 || size < 1 || keys < 0 || keys >= KEYS_COUNT ||
...
static void usage() {
  fprintf(stderr,
          "USAGE: random [-c] [-i size] [-k keys] [-p nthreads] [-s seed] [-w "
          "workload] pq\n"
          "       -c: Print performance counters (default = %d)\n"
          "       -i: Specifies the initial size of the priority queue "
          "(default = %d)\n"
          "       -k: Specifies the key generation type, one of %d: uniform, "
          "%d: ascending, %d: descending,"
          "           %d: restricted (8-bit), %d: restricted (16-bit) (default "
          "= %d)\n"
          "       -p: Specifies the number of threads (default = %d)\n"
          "       -s: Specifies the value used to seed the random number "
          "generator (default = %d)\n"
          "       -w: Specifies the workload type, one of %d: uniform, %d: "
          "split, %d: producer, %d: alternating (default = %d)\n"
          "       pq: The data structure to use as the backing priority queue\n"
          "           (one of '%s', '%s', '%s', '%s', '%s', '%s',\n"
          "                   '%s', '%s', '%s', '%s', '%s', '%s',\n"
          "                   '%s', '%s', '%s', '%s', '%s', '%s',\n"
          "                   '%s')\n",
          DEFAULT_COUNTERS, DEFAULT_SIZE, KEYS_UNIFORM, KEYS_ASCENDING,
          KEYS_DESCENDING, KEYS_RESTRICTED_8, KEYS_RESTRICTED_16, DEFAULT_KEYS,
          DEFAULT_NTHREADS, DEFAULT_SEED, WORKLOAD_UNIFORM, WORKLOAD_SPLIT,
          WORKLOAD_PRODUCER, WORKLOAD_ALTERNATING, DEFAULT_WORKLOAD, PQ_CADM,
          PQ_CAIN, PQ_CAPQ, PQ_CATREE, PQ_CHEAP, PQ_DLSM, PQ_GLOBALLOCK,
          PQ_KLSM16, PQ_KLSM128, PQ_KLSM256, PQ_KLSM4096, PQ_LINDEN, PQ_LSM,
          PQ_MLSM, PQ_MULTIQ, PQ_SEQUENCE, PQ_SKIP, PQ_SLSM, PQ_SPRAY);
  exit(EXIT_FAILURE);
}

class workload_uniform {
 public:
  workload_uniform(const struct settings &settings, const int thread_id)
...
class workload_split {
 public:
  workload_split(const struct settings &, const int thread_id)
...
class workload_producer {
 public:
  workload_producer(const struct settings &, const int thread_id)
      : m_thread_id(thread_id) {}
...
class workload_alternating {
 public:
  workload_alternating(const struct settings &, const int) : m_insert(0) {}
...
class keygen_uniform {
 public:
  keygen_uniform(const struct settings &settings, const int thread_id)
      : m_gen(settings.seed + thread_id),
        m_rand_int(0, std::numeric_limits<uint32_t>::max()) {}

  uint32_t next() { return m_rand_int(m_gen); }
...
class keygen_ascending {
 public:
  keygen_ascending(const struct settings &settings, const int thread_id)
      : m_gen(settings.seed + thread_id),
        m_rand_int(0, UPPER_BOUND),
        m_base(0) {}

  uint32_t next() { return m_rand_int(m_gen) + m_base++; }
...
class keygen_descending {
 public:
  keygen_descending(const struct settings &settings, const int thread_id)
      : m_gen(settings.seed + thread_id),
        m_rand_int(0, UPPER_BOUND),
        m_base(0) {}

  uint32_t next() { return MAX - m_rand_int(m_gen) - m_base++; }
...
class keygen_restricted {
 public:
  keygen_restricted(const struct settings &settings, const int thread_id)
      : m_gen(settings.seed + thread_id), m_rand_int(0, UPPER_BOUND) {}
...
static void bench_thread(PriorityQueue *pq, const int thread_id,
  ...
  /* Fill up to initial size. Do this per thread in order to build a balanced
   * DLSM instead of having one local LSM containing all initial elems. */
  ...
  const int initial_size = (thread_id == settings.nthreads - 1)
                               ? settings.size - thread_id * slice_size
                               : slice_size;
  ...
  while (!end_barrier.load(std::memory_order_relaxed)) {
    if (workload.insert()) {
      ...
      if (pq->delete_min(v)) {
#ifdef ENABLE_QUALITY
        deletions->emplace_back(
            packed_item_id{v.thread_id, v.element_id, rdtsc()});
#endif
...
#ifdef ENABLE_QUALITY
typedef std::pair<uint32_t, uint64_t>
    sort_t;  // <Incoming vector #, timestamp>.
...
static void sort_insertion_sequence(
    const std::vector<void *> &insertion_sequences,
...
static void sort_deletion_sequence(
    const std::vector<void *> &deletion_sequences,
...
static void evaluate_quality(std::vector<void *> &insertion_sequences,
                             std::vector<void *> &deletion_sequences,
                             double *mean, uint64_t *max, double *stddev) {
  ...
  assert(!global_deletion_sequence.empty() &&
         !global_insertion_sequence.empty());
  ...
  while (keep_running) {
    ...
    while (next_del_tick < next_ins_tick) {
      ...
      /* Look up the key. */
      auto insertions =
          (insertion_sequence_t *)insertion_sequences[deleted_item.thread_id];
...
static int bench(PriorityQueue *pq, const struct settings &settings) {
  ...
  switch (settings.workload) {
    case WORKLOAD_UNIFORM:
      switch (settings.keys) {
        case KEYS_UNIFORM:
          fn = bench_thread<PriorityQueue, workload_uniform, keygen_uniform>;
          break;
        case KEYS_ASCENDING:
          fn = bench_thread<PriorityQueue, workload_uniform, keygen_ascending>;
          break;
        case KEYS_DESCENDING:
          fn = bench_thread<PriorityQueue, workload_uniform, keygen_descending>;
          break;
        case KEYS_RESTRICTED_8:
          fn = bench_thread<PriorityQueue, workload_uniform,
                            keygen_restricted<1 << 8>>;
          break;
        case KEYS_RESTRICTED_16:
          fn = bench_thread<PriorityQueue, workload_uniform,
                            keygen_restricted<1 << 16>>;
          break;
        default:
          assert(false);
      }
    ...
    case WORKLOAD_SPLIT:
      switch (settings.keys) {
        case KEYS_UNIFORM:
          fn = bench_thread<PriorityQueue, workload_split, keygen_uniform>;
          break;
        case KEYS_ASCENDING:
          fn = bench_thread<PriorityQueue, workload_split, keygen_ascending>;
          break;
        case KEYS_DESCENDING:
          fn = bench_thread<PriorityQueue, workload_split, keygen_descending>;
          break;
        case KEYS_RESTRICTED_8:
          fn = bench_thread<PriorityQueue, workload_split,
                            keygen_restricted<1 << 8>>;
          break;
        case KEYS_RESTRICTED_16:
          fn = bench_thread<PriorityQueue, workload_split,
    ...
    case WORKLOAD_PRODUCER:
      switch (settings.keys) {
        case KEYS_UNIFORM:
          fn = bench_thread<PriorityQueue, workload_producer, keygen_uniform>;
          break;
        case KEYS_ASCENDING:
          fn = bench_thread<PriorityQueue, workload_producer, keygen_ascending>;
          break;
        case KEYS_DESCENDING:
          fn =
              bench_thread<PriorityQueue, workload_producer, keygen_descending>;
          break;
        case KEYS_RESTRICTED_8:
          fn = bench_thread<PriorityQueue, workload_producer,
                            keygen_restricted<1 << 8>>;
          break;
        case KEYS_RESTRICTED_16:
          fn = bench_thread<PriorityQueue, workload_producer,
    ...
    case WORKLOAD_ALTERNATING:
      switch (settings.keys) {
        case KEYS_UNIFORM:
          fn =
              bench_thread<PriorityQueue, workload_alternating, keygen_uniform>;
          break;
        case KEYS_ASCENDING:
          fn = bench_thread<PriorityQueue, workload_alternating,
                            keygen_ascending>;
          break;
        case KEYS_DESCENDING:
          fn = bench_thread<PriorityQueue, workload_alternating,
  ...
  uint64_t max;
  double mean, stddev;
  evaluate_quality(insertion_sequences, deletion_sequences, &mean, &max,
                   &stddev);
...
int main(int argc, char **argv) {
  ...
  struct settings settings = {
      DEFAULT_NTHREADS, DEFAULT_SEED, DEFAULT_SIZE,    "",
      DEFAULT_COUNTERS, DEFAULT_KEYS, DEFAULT_WORKLOAD};
  ...
  if (settings.type == PQ_CADM) {
    ...
    pq.insert((uint32_t)42,
              (uint32_t)42); /* A hack to avoid segfault on destructor in empty
                                linden queue. */
    ret = bench(&pq, settings);
