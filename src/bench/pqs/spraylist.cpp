...
void spraylist::init_thread(const size_t nthreads) {
  if (!initialized) {
...
void spraylist::insert(const uint32_t &k, const uint32_t &v) {
  sl_add_val(m_q, k, v, TRANSACTIONAL);
}

void spraylist::insert(const size_t &k, const size_t &v) {
  sl_add_val(m_q, k, v, TRANSACTIONAL);
}

bool spraylist::delete_min(size_t &k, size_t &v) {
  unsigned long k_ret;
  unsigned long v_ret;
  int ret;
  do {
...
bool spraylist::delete_min(uint32_t &v) {
  size_t k_ret;
  size_t v_ret;
