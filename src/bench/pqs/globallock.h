 ...
 private:
  class entry_t {
   public:
    ...
    bool operator>=(const entry_t &that) const { return !operator<(that); }
  };
...
template <class K, class V>
bool GlobalLock<K, V>::delete_min(K &key, V &value) {
...
template <class K, class V>
bool GlobalLock<K, V>::delete_min(V &value) {
  K key;
  return delete_min(key, value);
}

template <class K, class V>
void GlobalLock<K, V>::bubble_down(const size_t ix) {
  size_t i = ix;
  size_t j = (i << 1) + 1;
  while (j < m_length) {
    const size_t k = j + 1;
...
template <class K, class V>
void GlobalLock<K, V>::insert(const K &key, const V &value) {
  std::lock_guard<std::mutex> g(m_mutex);
...
template <class K, class V>
void GlobalLock<K, V>::bubble_up(const size_t ix) {
  size_t i = ix;
  size_t j = (i - 1) >> 1;
