...
class itree {
 ...
 private:
  ...
  typedef struct __itree_iter_t {
    ...
    itree_t *stack[ITREE_MAX_DEPTH]; /**< The node stack. Tree depth cannot be
                                        exceeded since keys are uint64_t. */
 ...
 public:
   ...
   */
  int insert(const elem_t &index);

  int erase(const elem_t &index, uint64_t *rank);
 ...
 private:
  ...
  void _itree_print(const itree_t *root, uint8_t level);
  int _itree_insert(const elem_t &index, itree_t **root, uint64_t *holes);
  int _itree_erase(const elem_t &index, itree_t **root, uint64_t *rank);
  int _itree_new_node(const elem_t &index, itree_t **root);
  void _itree_rebalance(itree_t **root);
  inline int8_t _itree_height(const itree_t *node);
  inline void _itree_set_height(itree_t *node);
  inline uint64_t _itree_count(const itree_t *root);
  int _itree_descend_l(const elem_t &index, itree_t **root, uint64_t *holes);
  int _itree_descend_r(const elem_t &index, itree_t **root, uint64_t *holes);
   ...
   */
  itree_iter_t *itree_iter_init(itree_t *root);
   ...
   *  * iter != NULL.
   */
  itree_t *itree_iter_next(itree_iter_t *iter);

  void itree_iter_free(itree_iter_t *iter);
