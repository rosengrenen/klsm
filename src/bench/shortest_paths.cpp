...
struct task_t {
  task_t(vertex_t *v, const size_t distance) : v(v), distance(distance) {
    num_tasks.fetch_add(1, std::memory_order_relaxed);
  }

  ~task_t() { num_tasks.fetch_sub(1, std::memory_order_relaxed); }
...
static void usage() {
  fprintf(
      stderr,
      "USAGE: shortest_paths [-m num_nodes] [-n num_threads] [-p "
      "edge_probability]\n"
      "                      [-s seed] pq\n"
      ...
      "           (one of '%s', %s', '%s', '%s')\n",
      DEFAULT_NNODES, DEFAULT_NTHREADS, DEFAULT_EDGE_P, DEFAULT_SEED, PQ_DLSM,
      PQ_GLOBALLOCK, PQ_KLSM, PQ_MULTIQ);
  exit(EXIT_FAILURE);
}

static vertex_t *generate_graph(const size_t n, const int seed,
                                const double p) {
...
static void verify_graph(const vertex_t *graph, const size_t n) {
  for (size_t i = 0; i < n; i++) {
...
template <class T>
static void bench_thread(T *pq, const int thread_id, vertex_t *graph) {
...
static int bench(T *pq, const struct settings &settings) {
  ...
  int ret = 0;
  vertex_t *graph = generate_graph(settings.num_nodes, settings.seed,
                                   settings.edge_probability);
...
int main(int argc, char **argv) {
  int ret = 0;
  struct settings s = {DEFAULT_NNODES, DEFAULT_NTHREADS, DEFAULT_EDGE_P,
                       DEFAULT_SEED, ""};
