...
class dist_lsm_local {
 public:
  ...
  bool delete_min(dist_lsm<K, V, Rlx> *parent, V &val);
  bool delete_min(dist_lsm<K, V, Rlx> *parent, K &key, V &val);
 ...
 private:
   ...
   */
  void merge_insert(block<K, V> *const new_block, shared_lsm<K, V, Rlx> *slsm);
