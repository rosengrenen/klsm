...
template <class K, class V, int Rlx>
void dist_lsm_local<K, V, Rlx>::insert(const K &key, const V &val,
...
template <class K, class V, int Rlx>
void dist_lsm_local<K, V, Rlx>::insert(item<K, V> *it, const version_t version,
                                       shared_lsm<K, V, Rlx> *slsm) {
...
template <class K, class V, int Rlx>
void dist_lsm_local<K, V, Rlx>::merge_insert(block<K, V> *const new_block,
                                             shared_lsm<K, V, Rlx> *slsm) {
  ...
  while (other_block != nullptr &&
         insert_block->capacity() == other_block->capacity()) {
    /* Only merge into a larger block if both candidate blocks have enough
     * elements to justify the larger size. This change is necessary to avoid
     * huge blocks containing only a few elements (which actually happens with
     * the 'alloc largest block on insert' optimization. */
    const size_t merged_pow2 =
        (insert_block->size() + other_block->size() <= insert_block->capacity())
            ? insert_block->power_of_2()
            : insert_block->power_of_2() + 1;
...
template <class K, class V, int Rlx>
bool dist_lsm_local<K, V, Rlx>::delete_min(dist_lsm<K, V, Rlx> *parent, K &key,
                                           V &val) {
...
template <class K, class V, int Rlx>
bool dist_lsm_local<K, V, Rlx>::delete_min(dist_lsm<K, V, Rlx> *parent,
                                           V &val) {
  K key;
...
template <class K, class V, int Rlx>
void dist_lsm_local<K, V, Rlx>::peek(typename block<K, V>::peek_t &best) {
  /* Short-circuit. */
  ...
  for (auto i = m_head.load(std::memory_order_relaxed); i != nullptr;
    ...
    while (i->size() <= i->capacity() / 2) {
      ...
      if (next != nullptr && new_block->capacity() == next->capacity()) {
        auto merged_block =
            m_block_storage.get_block(new_block->power_of_2() + 1);
...
template <class K, class V, int Rlx>
void dist_lsm_local<K, V, Rlx>::safe_peek(typename block<K, V>::peek_t &best) {
  for (auto i = m_head.load(std::memory_order_relaxed); i != nullptr;
       i = i->m_next.load(std::memory_order_relaxed)) {
    auto candidate = i->peek();
...
template <class K, class V, int Rlx>
int dist_lsm_local<K, V, Rlx>::spy(dist_lsm<K, V, Rlx> *parent) {
...
template <class K, class V, int Rlx>
int dist_lsm_local<K, V, Rlx>::spy(dist_lsm_local<K, V, Rlx> *victim) {
  if (m_tail != nullptr) {
...
template <class K, class V, int Rlx>
void dist_lsm_local<K, V, Rlx>::print() const {
