...
multi_lsm<K, V, C>::multi_lsm(const size_t num_threads)
    : m_num_threads(num_threads), m_num_queues(num_threads * C) {
...
template <class K, class V, int C>
void multi_lsm<K, V, C>::insert(const K &key) {
  insert(key, key);
}

template <class K, class V, int C>
void multi_lsm<K, V, C>::insert(const K &key, const V &val) {
...
template <class K, class V, int C>
bool multi_lsm<K, V, C>::delete_min(V &val) {
